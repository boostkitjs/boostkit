...
- [x] The smoke prompt *"bold the word 'lorem' in the content, add a callToAction at the end with title 'Subscribe', and convert the first paragraph to an h1"* completes in one turn with mixed `update_form_state` + `edit_text` calls and no HTTP 400. **Verified manually 2026-04-08.** Chat displayed both tool calls inline (`Updated content (update_form_state)` + `Updated content (edit_text)`), `Done — 1 step`, and the model wrapped up with a natural-language confirmation that knows both tools succeeded — meaning the server tool result reached its context on the continuation pass.
